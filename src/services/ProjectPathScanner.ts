...
export class ProjectPathScanner {
  ...
  async scanProject(
  ...
  ): Promise<ProjectPathData | null> {
    const startTime = Date.now();
    logger.info(
      "ProjectPathScanner",
      `Scanning project ${workspaceFolder.name}...`,
    );
    ...
    try {
      ...
      for (let i = 0; i < files.length; i += SCAN_CONCURRENCY) {
        ...
        const batchResults = await Promise.all(
          batch.map(async (fileUri) => {
            try {
              return await this.parseVerseFile(
                fileUri,
                workspaceFolder,
                options,
              );
      ...
      logger.info(
        "ProjectPathScanner",
        `Scanned ${nodes.length} declarations from ${files.length} files in ${elapsed}ms`,
      );
  ...
  async parseVerseFile(
    ...
    try {
      ...
      logger.error(
        "ProjectPathScanner",
        `Failed to parse ${fileUri.fsPath}`,
        error,
      );
      return [];
    }
  ...
  extractDeclarations(
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      while (
        ...
        indent <= moduleStack[moduleStack.length - 1].indent
      ) {
        moduleStack.pop();
      }
      currentModulePath =
        moduleStack.length > 0 ? moduleStack[moduleStack.length - 1].name : "";
      ...
      if (moduleMatch) {
        ...
        const fullPath = currentModulePath
          ? `${currentModulePath}.${name}`
          : name;
