...
describe("buildProjectIndexes", () => {
  it("indexes identifiers case-insensitively but module names exactly", () => {
    const nodes = [
      moduleNode("Inventory", "Inventory", "Content/File.verse"),
      classNode("Widget", "Widget", "Content/File.verse"),
    ];
  ...
  it("only includes module declarations in the module name index", () => {
    const nodes = [
      classNode("Inventory", "Systems.Inventory", "Content/File.verse"),
      moduleNode("Systems", "Systems", "Content/File.verse"),
    ];
  ...
  it("groups nodes by source file", () => {
    const nodes = [
      moduleNode("A", "A", "Content/One.verse"),
      moduleNode("B", "B", "Content/One.verse"),
      moduleNode("C", "C", "Content/Two.verse"),
    ];
...
describe("resolveModuleLocations", () => {
  it("resolves a top-level module declared in a file at the Content root", () => {
    ...
    expect(resolve("Inventory", nodes)).toEqual([
      { location: "", sourceFile: "Content/Main.verse" },
    ]);
  });
  ...
  it("resolves a module declared in a subdirectory to that directory prefix", () => {
    ...
    expect(resolve("Inventory", nodes)).toEqual([
      { location: "/Systems", sourceFile: "Content/Systems/Main.verse" },
    ]);
  });
  ...
  it("never returns non-module declarations", () => {
    const nodes = [
      classNode("Inventory", "Inventory", "Content/Main.verse"),
      classNode("Inventory", "Systems.Inventory", "Content/Other.verse"),
    ];
  ...
  it("does not match a nested module by its bare name", () => {
    ...
    const nodes = [
      moduleNode("Inventory", "Systems.Inventory", "Content/Main.verse"),
    ];
  ...
  it("resolves a module nested inside another module in the same file", () => {
    ...
    expect(resolve("HUD/Textures", nodes)).toEqual([
      { location: "/Foo", sourceFile: "Content/Foo/UI.verse" },
    ]);
  });
  ...
  it("resolves remaining segments against the file's directory tail (implicit folder modules)", () => {
    ...
    const nodes = [
      moduleNode("Textures", "Textures", "Content/HUD/Textures.verse"),
    ];

    expect(resolve("HUD/Textures", nodes)).toEqual([
      { location: "", sourceFile: "Content/HUD/Textures.verse" },
    ]);
  });

  it("rejects candidates whose directory does not match the remaining segments", () => {
    const nodes = [
      moduleNode("Textures", "Textures", "Content/Other/Textures.verse"),
    ];
  ...
  it("treats source paths as Content-relative when the workspace is the Content folder", () => {
    ...
    expect(resolve("Inventory", nodes, true)).toEqual([
      { location: "/Systems", sourceFile: "Systems/Main.verse" },
    ]);
  ...
  it("resolves files directly at the workspace root when the workspace is Content", () => {
    ...
    expect(resolve("Inventory", nodes, true)).toEqual([
      { location: "", sourceFile: "Main.verse" },
    ]);
  });
