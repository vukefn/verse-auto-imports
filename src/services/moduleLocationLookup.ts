...
export function buildProjectIndexes(
  nodes: readonly ProjectPathNode[],
): ProjectIndexes {
...
export function resolveModuleLocations(
  ...
  options: { workspaceIsContent: boolean },
  ...
  for (const node of candidates) {
    ...
    const contentRelativeDir = toContentRelativeDir(
      node.sourceFile,
      options.workspaceIsContent,
    );
    if (contentRelativeDir === null) {
      continue;
    }

    const dirSegments = contentRelativeDir
      .split("/")
      .filter((s) => s.length > 0);
    ...
    for (let i = 0; i < remaining.length; i++) {
      if (
        dirSegments[dirSegments.length - remaining.length + i] !== remaining[i]
      ) {
        dirMatches = false;
        break;
      }
    ...
    const locationSegments = dirSegments.slice(
      0,
      dirSegments.length - remaining.length,
    );
    const location =
      locationSegments.length > 0 ? "/" + locationSegments.join("/") : "";
