...
describe("ImportSuggestionExtractor", () => {
  ...
  describe("extractImportSuggestions", () => {
    it("should parse 'Did you forget to specify using { /Path }' into a high-confidence suggestion", async () => {
      const errorMessage =
        "This identifier is unknown. Did you forget to specify using { /Verse.org/Simulation }";

      const suggestions =
        await extractor.extractImportSuggestions(errorMessage);

      expect(suggestions).toHaveLength(1);
      expect(suggestions[0].importStatement).toBe(
